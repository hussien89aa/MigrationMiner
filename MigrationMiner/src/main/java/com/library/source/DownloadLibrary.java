...
public class DownloadLibrary {
	...
	//static String pathClassSignature=Paths.get(".").toAbsolutePath().normalize().toString() +"/LibrariesDocs"; // +"/librariesClasses/";
	public static void main(String[] args) {
		...
		//System.out.println( "Is library found:"+	new DownloadLibrary().isLibraryFound(libraryInfo));
	...
	public	boolean isLibraryFound(String LibraryInfo){
		...
		if(LibraryInfos.length<3){ return isFound;}
	...
	public void generateFunctionSignature(String libraryName,String pathToSaveLibrarySignature){
		...
		deleteFile(pathToSaveLibrarySignature +libraryName +".txt");
				 ...
				 try{
					 ...
					 String cmdStr="cd "+ pathToSaveLibrarySignature +" && javap -classpath jar/"+libraryName +" $(jar -tf jar/"+libraryName +" | grep \"class$\" | sed s/\\.class$//) >>"+ libraryName +".txt";
