...
/**
 * This file responsible for collect all added and removed libraries in all commits for list of github project
 *
 */
			...
			//if (gitHubOP.isPageExisit(appURL+"/blob/master/pom.xml")){
			 ...
			 // save this link in clean repo links
			    ...
			    for (int i=0;i<commitList.size();i++){
			    	...
			    	/* we use commitInfo.filePath instead this
			    	ArrayList<String> listOfPaths= pomFilePath.findPomFilePath(pathClone, appFolder);
			    	if(listOfPaths.size()==0){
			    		System.out.println("Cannot find pom file in this commit");
			    		continue;
			    	}*/
			    	...
			    	for (CommitFiles pomPath : commitInfo.filePath) {
			    	 	// make sure all commits has init library
			    		  ...
			    		  String prevCommitLibraries= generateAppLibraries( appFolder+"/"+pomPath.firstFile, projectID, commitInfo.commitID, previousVersionLibraries.get(pomPath.firstFile));
         ...
         if(listOfRemovedLibrary.size()>0){
         	for (String libraryName : listOfRemovedLibrary) {
             	System.err.println("Removed:"+libraryName);
                	new ProjectLibrariesDB().addProjectLibrary(projectID, commitID, libraryName,Operation.removed,pomPath);	
				}
         }
         ...
         if(listOfAddedLibrary.size()>0){
         	for (String libraryName : listOfAddedLibrary) {
             	System.err.println("Added:"+libraryName);
                	new ProjectLibrariesDB().addProjectLibrary(projectID, commitID, libraryName,Operation.added,pomPath);	
				}
         }
	...
	public	ArrayList<String> listOfChangedLibrary(String previousVersionLibraries,String versionLibraries){
	    ...
	    if(previousVersionLibraries!=versionLibraries){
	    	...
	    	for (String prevLibrary : prev) {
	    		if(prevLibrary.length()==0){continue;};
...
//	    }
	...
	public String listOfJavaProjectLibrary(String projectVersionPath){
	    ...
	    try {
	          ...
	          /*DEBUG
	        for (String element : propertiesList.keySet()) {
