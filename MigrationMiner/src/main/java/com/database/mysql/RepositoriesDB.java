...
public class RepositoriesDB {
	...
	public void addRepository(String AppLink, String AppType) {
		...
		try {
			...
			String sql = "INSERT INTO Repositories (AppLink,AppType) " + "VALUES ('" + AppLink + "', '" + AppType
					+ "');";
	...
	public String getRepositoriesLink(int AppID) {
		...
		try {
			...
			c.setAutoCommit(false);
			stmt = c.createStatement();
			ResultSet rs = stmt.executeQuery("Select AppLink from Repositories where AppID=" + AppID); // TODO order by
																										// ProjectsID,
																										// commitID"
	...
	public ArrayList<Repository> getRepositories() {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery(
					"SELECT * from Repositories  where AppID in(SELECT AppID FROM MigrationSegments GROUP BY AppID)"); // where
																														// ProjectsID=192"

			while (rs.next()) {

				listOfRepositories.add(new Repository(rs.getInt("AppID"), rs.getString("AppLink")));
			}
			rs.close();
			stmt.close();
			c.close();
